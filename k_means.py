...
if page == "Xử lý dữ liệu":
    ...
    if uploaded_file is not None:
        if not uploaded_file.name.endswith(".csv"):
            ...
            st.markdown("<h5 style='text-align: left; color:#47B5FF'>&#9679; Xóa bỏ các cột không quan trọng </h5>", unsafe_allow_html=True)
            ...
            st.markdown("<h5 style='text-align: left; color:#47B5FF'>&#9679; Xử lý các giá trị Null và các cột định dạng Object </h5>", unsafe_allow_html=True)
            st.markdown("<p style='text-align: left; color:while'>\"<b>Xóa bỏ</b> các hàng hoặc cột dữ liệu chứa giá trị Null "
                        "tùy vào số lượng giá trị Null trong cột đó.\"</p>", unsafe_allow_html=True)
            st.markdown("<p style='text-align: left; color:while'>\"<b>Mã hóa</b> các cột dữ liệu thuộc định dạng Object "
                        "thành dạng số nguyên.\"</p>", unsafe_allow_html=True)
            # Xử lý các cột chứa giá trị null
            if st.button('Xử lý dữ liệu'):
                ...
                st.success('\u2705 Dữ liệu chứa giá trị Null và định dạng Object đã được xử lý xong.')
                st.dataframe(data_processing)
                st.markdown("<p style='text-align: center; color:while'>\u2705 Chuyển sang trang mới để tiếp tục !</p>", unsafe_allow_html=True)
    ...
    cat_cols_check = data_elbow.select_dtypes(include=['object']).columns.tolist()
    if data_elbow.empty:
        st.warning("Bạn cần chọn file dữ liệu trước")
    if data_elbow.isnull().values.any() or data_elbow.isna().values.any() and len(cat_cols_check) > 0:
        st.write('\u274C Dữ liệu chưa được xử lý, vui lòng xử lý dữ liệu trước khi thao tác!')
    else:
        ...
        st.markdown("<h6 style='text-align: left; color:#47B5FF'>&#9679; Biểu đồ khuỷu tay để tìm số cụm tối ưu</h6>", unsafe_allow_html=True)
            ...
            try:
                ...
                if k:
                    try:
                        k = int(k)
                        if k > 10:
                            st.warning("Số k cụm phải nhỏ hơn hoặc bằng 10. Vui lòng nhập lại!")
                        else:
                ...
                st.error("Dữ liệu của bạn có vấn đề!")
        else:
        ...
        if num_cols == 2:
            try:
                if len(columns) >= 2:
                    ...
                    ax.scatter(centers[:, 0], centers[:, 1], c='black', s=100, alpha=0.4)
                    ax.scatter(centers[:, 0], centers[:, 1], c='black', s=110, alpha=0.4, edgecolor='red')
